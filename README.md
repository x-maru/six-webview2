...
- `install.cmd`を実行するとデスクトップに起動用ショートカット`six`が作成される。
- フォルダを移したりした場合は再度`install.cmd`を実行する。
(`install.cmd`は単にショートカットを作成するだけ)
...
- `e`,`E`,`ge`,`gE`の移動先がword/WORDの末尾ではなくその1つ後ろに飛ぶ。`$`でも行末の改行コードにcaretが乗る(Vim,標準viではその手前)
...
- 「INSERTからNORMALに移行したときに自動的にIMEをoffにする」もトグルだと中途半端
- IME入力中の`Esc`は即、sixが消費して「IME off&NORMALモードへ移行」の動きになる。
...
- フォント
  - フリーのフォント「SarasaMonoJ(更紗ゴシック)」がインストールされていれば優先的に使用する。[GitHub](https://github.com/be5invis/Sarasa-Gothic)
    - `SarasaMonoJ-TTF-1.0.35.7z`をダウンロードして中のTTFファイルをインストールする必要がある。

## ヘルプ(外部Markdown)
- `_six.js`と同じフォルダに`任意名.NN.md`(NN=01..99) を置くと、ヘルプダイアログの各タブを外部Markdownから読み込む。
  - NNの昇順でタブに割り当て。
  - タブ名はMarkdownの1行目（先頭の`# `は除去）。
- 環境によってはフォルダ内のファイル一覧取得ができないことがある。
  - その場合は`help.index.txt`または`help.index.json`を同フォルダに置くと、そこに列挙したファイル名を読み込む。
    - `help.index.txt`: 1行1ファイル。空行と`#`/`//`で始まる行は無視。
    - `help.index.json`: `[{"...":...}]`は未対応。`["foo.01.md","bar.02.md"]`または`{ "files": [ ... ] }`をサポート。
...
- v0.9→v0.92
  ...
  - ジャンプ系以外の行スクロールを4px単位のスムーズスクロールにした。
