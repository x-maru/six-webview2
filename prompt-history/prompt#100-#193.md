...
#188
- :w pathの上書き確認 → 動作OKだけど「このページの内容:」って表示されないようにしたい。
- :w! path → OK
- :wq → OK
- :wq! → 不具合はないが、編集済みバッファが複数あるときに全保存/全キャンセルではなく、ひとつずつダイアログを出すようにしたい。
- :wq path → OK
- :wで未編集だったとき一瞬EOFが見えるところまでジャンプして戻るのを抑止したい。
- :qでmodifiedがあるときの確認 → 動作OKだけど「このページの内容:」って表示されないようにしたい。
- 保存時に改行の種類を変更する機能は不要。カレントバッファの改行を変更する機能はいずれ設ける(TODO)。なので保存時の改行コードは「変更せずそのまま保存」を仕様とする。

#189
- :w path 上書き確認
  - cancel/overwriteをclickしたあとフォーカスを失って:も何も受け付けない。
  - dialogを開いた時にoverwriteにフォーカス表示が付いたり付かなかったりする。常に付いた方がいい。
- :q の未保存確認
  - cancelをclickしたあとフォーカスを失って:も何も受け付けない。
- :wq（!なし）
  - 不具合はないが仕様変更したい。
    - 未保存の全バッファを列挙してそれぞれに【save】【Don't save】のボタンを設けたい。
    - 【all save】【cancel】ボタンを1組だけ各バッファ行より上に表示。
    - save / don't saveを選択したバッファ行はdialogから消していく(dialogが短くなっていく)。
    - 未保存バッファが多い場合に画面からはみ出てもOK。スクロールバーは出なくてもよい。
      - わざわざ出るようにしなくてもいいというだけなので普通にバーが出るならそれでいい。
    - 未保存バッファが1つだけのときも同じUI。
- :w（未編集）位置復元により EOF が一瞬見える現象を抑止。
  - 変化がわからない。一瞬とはいえはっきり見えてしまっている。

#190
- :w exist.txt
  - 概ねOK。overwriteボタンにフォーカスが付くのはよいが、Tabでどこかに行ってしまいShift+Tabでも戻ってこない。フォーカスはcancelとoverwriteの2箇所だけに絞りたい。
  - ":wa"と入力すると":w a"として扱われてしまう。wa未実装なら不正コマンドとして扱うべき。
- :q (未保存バッファあり)
  - 将来的にsplitを実装したら:qは「バッファ破棄&split解除」になるだろうし、現段階で:qの動作は「必要なら保存した上で現バッファを破棄する」だけにしたい。「バッファ破棄 = タブ削除」。その結果バッファ(タブ)が0になったら確認無しでsixを終了するが、他バッファ(タブ)が残っていれば次のタブをアクティブにするだけ。:q!でも対象はカレントバッファのみ。
  - overwrite dialogと同様、3つのボタン間だけにキーボードフォーカスを絞りたい。
- :wq (未保存バッファ複数あり)
  - "All save"の隣に「すべて破棄」のボタンも追加したい。【Discard all】かな。
  - overwrite dialogと同様、3つ(プラス1つ追加)のボタン間だけにキーボードフォーカスを絞りたい。
- :w（未編集）
  - gg → :w でGの挙動になって表示範囲が変わったまま戻らなくなってしまった。
...
- :wa
  - NG。":w a"として扱われてしまう。
- :q
  - :qaを実装して、今の:qの挙動を:qaにしよう。
  - :qはカレントバッファだけを対象にする。結果としてバッファがなくなったときだけsixを終了する
- :wq
  - :wqaを実装して、今の:wqの挙動を:wqaにしよう。
  - :wqはカレントバッファだけを対象にする。結果としてバッファがなくなったときだけsixを終了する
- :w（未編集）
  - :w でGの挙動になって表示範囲が変わったまま戻らない現象は改善されていない。
- 引数なしで起動したときの初期バッファで最初からGで飛んだ位置が表示される (NG)
  - six.ps1 _six.cssでは正常に行1から表示される
- `six.ps1 _six.js`で起動すると下記のエラーになり、開いたバッファは空になっている。
```
six.ps1 starting in: C:\Users\ymaru\WebView2\six
"1" 個の引数を指定して "EscapeDataString" を呼び出し中に例外が発生しました: "無効な
 URI: Uri の文字列が長すぎます。"
発生場所 C:\Users\ymaru\WebView2\six\six.ps1:156 文字:3
...
#192
- :q
  - Cancelで抜けたあとコマンド入力欄にフォーカスが残ってしまう
  - カレントバッファのみに効く → OK
  - バッファがなくなったらsix終了 → OK
- :wa
  - OK
- :qa
  - ダイアログを:wqaのものと同じにしよう。
    - :wqaはカレントバッファを保存して閉じてから他にまだあればダイアログを出す、:qaはカレントバッファが編集中でなければ閉じ、他にまだあればダイアログを出す。
- :wqa
  - 未保存バッファがカレント以外にある場合にダイアログを出す前にカレントバッファを保存して閉じるべき。今はカレントバッファを閉じずにダイアログが開いている。
- :w（未編集）
  - :w でGの挙動になって表示範囲が変わったまま戻らない現象は改善されていない。
- 次タスク
  - `six.ps1 _six.js`で起動すると下記のエラーになり、開いたバッファは空になっている。
...
- :qa → OK
- :wqa → OK
- :w → NG。戻ってくるようにはなったが、一瞬EOFに飛んでいる。
- `six.ps1 _six.js`で起動すると下記のエラーになり、開いたバッファは空になっている。
```
six.ps1 starting in: C:\Users\ymaru\WebView2\six
"1" 個の引数を指定して "EscapeDataString" を呼び出し中に例外が発生しました: "無効な
